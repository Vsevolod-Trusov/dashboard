...
export const staffValidationSchema = object().shape({
    email: string().required(VALIDATION_ERRORS.INVALID_EMAIL).email(VALIDATION_ERRORS.INVALID_EMAIL),
    ...
    password: string().required(VALIDATION_ERRORS.INVALID_PASSWORD).min(8, VALIDATION_ERRORS.MIN_PASSWORD),
    confirm: string().required(VALIDATION_ERRORS.REQUIRED_CONFIRM).min(8, VALIDATION_ERRORS.MIN_PASSWORD),
    isManager: boolean()
  });
